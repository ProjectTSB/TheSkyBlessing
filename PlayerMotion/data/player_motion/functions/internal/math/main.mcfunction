...
# full_power=sqrt(motion_x²+motion_y²+motion_z²)
	...
	execute store success score $negative_sine player_motion.internal.math if score $motion_y player_motion.internal.math matches ..-1

    # Normalize
        execute if score $motion_x player_motion.internal.math matches ..-1 run scoreboard players operation $motion_x player_motion.internal.math *= #constant.-1 player_motion.internal.const

        execute if score $motion_y player_motion.internal.math matches ..-1 run scoreboard players operation $motion_y player_motion.internal.math *= #constant.-1 player_motion.internal.const

        execute if score $motion_z player_motion.internal.math matches ..-1 run scoreboard players operation $motion_z player_motion.internal.math *= #constant.-1 player_motion.internal.const

    # Sets angle before exchange
        scoreboard players set $angle player_motion.internal.math 0

        execute if score $motion_y player_motion.internal.math matches 0 run scoreboard players set $angle player_motion.internal.math 1

    # SuperSwordTW's method. Credit: https://github.com/SuperSwordTW/Distance-Trig-Calc-3d
        execute if score $motion_y player_motion.internal.math matches 0 run scoreboard players operation $motion_y player_motion.internal.math >< $motion_x player_motion.internal.math

        execute if score $motion_y player_motion.internal.math matches 0 run scoreboard players operation $motion_y player_motion.internal.math >< $motion_z player_motion.internal.math
...
# sub power operations
    ...
    # d += sine
    execute store result storage player_motion:math d double -0.0001 run scoreboard players operation $d player_motion.internal.math += $sine player_motion.internal.math
...
# full power operations
    ...
    # d += sine1
    execute store result storage player_motion:math full_d double -0.0001 run scoreboard players operation $d player_motion.internal.math += $sine1 player_motion.internal.math
