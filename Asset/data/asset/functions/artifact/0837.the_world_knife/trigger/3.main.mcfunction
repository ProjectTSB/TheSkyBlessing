...
# 弾を召喚（敵がいる場合）
    execute anchored eyes positioned ^ ^ ^2 if entity @e[type=#lib:living,tag=N9.Target,distance=..4,limit=1] at @e[type=#lib:living,tag=N9.Target,distance=..4,limit=1] run function asset:artifact/0837.the_world_knife/trigger/knife_summon_execute

# 弾を召喚(敵がいない場合)
    execute anchored eyes positioned ^ ^ ^2 unless entity @e[type=#lib:living,tag=N9.Target,distance=..4,limit=1] positioned ^ ^ ^-2 positioned ~ ~-0.7 ~ run function asset:artifact/0837.the_world_knife/trigger/knife_summon_forward
...
# Scheduleループ
    schedule function asset:artifact/0837.the_world_knife/trigger/4.tick 1t replace