#> asset:artifact/0077.swords_of_waterfall_climbing/give/2.give
#
# 神器の作成部 ここでID等を定義する
#
# @user
# @within function asset:artifact/0077.swords_of_waterfall_climbing/give/1.trigger

# 神器の説明や消費MPなどをここで設定する。
# 最後にasset:artifact/common/giveを実行することで入手可能。

# 神器のID (int) スプレッドシートの値を入れる
    data modify storage asset:artifact ID set value 77
# 神器のベースアイテム
...
# 神器の説明文 (TextComponentString[])
    data modify storage asset:artifact Lore set value ['{"text":"滝を登るかの如く、敵と自分を上に飛ばす剣。"}','{"text":"敵に水属性ダメージを与える"}']
...
# グローバルクールダウン (int) (オプション)
    # data modify storage asset:artifact GlobalCooldown set value
# 扱える神 (string[]) Wikiを参照
