#> asset:spawner/099/register
#
# スポナーの定義データ
#
# @within function asset:spawner/099/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:overworld,X:101,Y:11,Z:149}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:47,Weight:2},{Id:217,Weight:1}]
# 一度に召喚する数 (int)
