#> asset:spawner/330/register
#
# スポナーの定義データ
#
# @within function asset:spawner/330/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:overworld,X:-167,Y:75,Z:42}

# ID (int)
    data modify storage asset:spawner ID set value 330
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [81,49]
# 一度に召喚する数 (int)
