...
# スポナーの定義データ
#
# @within function asset:spawner/145/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:overworld,X:446,Y:165,Z:71}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:141,Weight:2},{Id:142,Weight:2},{Id:233,Weight:2},{Id:232,Weight:1}]
# 一度に召喚する数 (int)
...
# 最低召喚間隔 (int)
    data modify storage asset:spawner MinSpawnDelay set value 250
# 最大召喚間隔 (int)
