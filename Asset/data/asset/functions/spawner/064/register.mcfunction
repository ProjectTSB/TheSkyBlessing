#> asset:spawner/064/register
# 
# スポナーの定義データ
# 
# @within function asset:spawner/064/


# 重複防止レジストリに登録
    data modify storage asset:spawner DPR append value {D:overworld,X:143,Y:8,Z:143}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [37,17,107]
# 一度に召喚する数 (int)
