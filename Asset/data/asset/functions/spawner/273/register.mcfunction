...
# スポナーの定義データ
# 
# @within function asset:spawner/273/


# 重複防止レジストリに登録
    data modify storage asset:spawner DPR append value {D:overworld,X:-189,Y:90,Z:-189}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [49,47,166]
# 一度に召喚する数 (int)
