...
# スポナーの定義データ
#
# @within function asset:spawner/230/


# 重複防止レジストリに登録
    data modify storage asset:spawner DPR append value {D:overworld,X:-23,Y:105,Z:-137}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [37,89]
# 一度に召喚する数 (int)
