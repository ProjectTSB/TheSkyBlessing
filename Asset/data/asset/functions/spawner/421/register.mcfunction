...
# スポナーの定義データ
#
# @within function asset:spawner/421/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:overworld,X:22,Y:197,Z:366}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:83,Weight:2},{Id:190,Weight:2},{Id:210,Weight:1}]
# 一度に召喚する数 (int)
