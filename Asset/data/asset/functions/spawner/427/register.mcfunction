...
# スポナーの定義データ
#
# @within function asset:spawner/427/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:overworld,X:53,Y:156,Z:337}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:181,Weight:2},{Id:264,Weight:1}]
# 一度に召喚する数 (int)
