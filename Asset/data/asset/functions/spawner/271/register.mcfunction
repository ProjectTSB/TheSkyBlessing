...
# スポナーの定義データ
# 
# @within function asset:spawner/271/


# 重複防止レジストリに登録
    data modify storage asset:spawner DPR append value {D:overworld,X:-122,Y:49,Z:-122}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [178,179]
# 一度に召喚する数 (int)
