...
# スポナーの定義データ
#
# @within function asset:spawner/389/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:overworld,X:308,Y:46,Z:276}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [287,155]
# 一度に召喚する数 (int)
