...
# スポナーの定義データ
# 
# @within function asset:spawner/191/


# 重複防止レジストリに登録
    data modify storage asset:spawner DPR append value {D:overworld,X:152,Y:82,Z:152}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [16,37]
# 一度に召喚する数 (int)
