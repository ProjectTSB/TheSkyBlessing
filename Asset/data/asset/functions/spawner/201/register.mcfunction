...
# スポナーの定義データ
#
# @within function asset:spawner/201/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:overworld,X:36,Y:67,Z:269}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [147,146]
# 一度に召喚する数 (int)
