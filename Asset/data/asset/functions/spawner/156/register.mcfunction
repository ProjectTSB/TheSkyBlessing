...
# スポナーの定義データ
#
# @within function asset:spawner/156/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:overworld,X:416,Y:144,Z:41}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:233,Weight:2}]
# 一度に召喚する数 (int)
