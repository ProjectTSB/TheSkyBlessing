#> asset:spawner/035/register
# 
# スポナーの定義データ
# 
# @within function asset:spawner/035/


# 重複防止レジストリに登録
    data modify storage asset:spawner DPR append value {D:overworld,X:216,Y:12,Z:216}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [21,26]
# 一度に召喚する数 (int)
