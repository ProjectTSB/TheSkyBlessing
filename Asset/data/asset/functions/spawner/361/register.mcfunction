...
# スポナーの定義データ
#
# @within function asset:spawner/361/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:overworld,X:-185,Y:129,Z:219}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [171,161]
# 一度に召喚する数 (int)
