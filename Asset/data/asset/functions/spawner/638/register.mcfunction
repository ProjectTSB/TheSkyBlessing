...
# スポナーの定義データ
#
# @within function asset:spawner/638/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:overworld,X:-23,Y:193,Z:405}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [181,83,210]
# 一度に召喚する数 (int)
