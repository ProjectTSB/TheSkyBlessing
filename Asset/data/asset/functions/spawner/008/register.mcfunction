...
# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR set value {D:overworld,X:-46,Y:14,Z:42}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [21,38]
# 一度に召喚する数 (int)
