#> asset:spawner/033/register
# 
# スポナーの定義データ
# 
# @within function asset:spawner/033/


# 重複防止レジストリに登録
    data modify storage asset:spawner DPR append value {D:overworld,X:163,Y:12,Z:163}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [21,64]
# 一度に召喚する数 (int)
