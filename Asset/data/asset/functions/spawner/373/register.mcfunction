...
# スポナーの定義データ
#
# @within function asset:spawner/373/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:overworld,X:-218,Y:102,Z:220}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [257,47]
# 一度に召喚する数 (int)
