#> asset:spawner/009/register
#
# スポナーの定義データ
#
# @within function asset:spawner/009/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:overworld,X:-41,Y:38,Z:-92}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [62,7]
# 一度に召喚する数 (int)
