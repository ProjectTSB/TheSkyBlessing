#> asset:spawner/096/register
#
# スポナーの定義データ
#
# @within function asset:spawner/096/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:overworld,X:104,Y:25,Z:161}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:47,Weight:1},{Id:53,Weight:1}]
# 一度に召喚する数 (int)
