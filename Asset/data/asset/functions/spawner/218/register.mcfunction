...
# スポナーの定義データ
#
# @within function asset:spawner/218/


# 重複防止レジストリに登録
    data modify storage asset:spawner DPR append value {D:overworld,X:44,Y:126,Z:-157}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [16,15]
# 一度に召喚する数 (int)
