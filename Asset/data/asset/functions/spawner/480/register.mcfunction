...
# スポナーの定義データ
#
# @within function asset:spawner/480/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:overworld,X:-104,Y:196,Z:-25}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [49,124]
# 一度に召喚する数 (int)
