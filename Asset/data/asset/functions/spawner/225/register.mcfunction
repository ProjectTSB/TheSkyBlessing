...
# スポナーの定義データ
#
# @within function asset:spawner/225/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:overworld,X:-100,Y:202,Z:247}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [181,60]
# 一度に召喚する数 (int)
