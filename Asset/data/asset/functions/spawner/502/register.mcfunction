...
# スポナーの定義データ
#
# @within function asset:spawner/502/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:overworld,X:178,Y:159,Z:-203}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [274,274,273,273]
# 一度に召喚する数 (int)
