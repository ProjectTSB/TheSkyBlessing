...
# スポナーの定義データ
# 
# @within function asset:spawner/272/


# 重複防止レジストリに登録
    data modify storage asset:spawner DPR append value {D:overworld,X:-139,Y:52,Z:-139}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [179,177]
# 一度に召喚する数 (int)
