#> asset:spawner/078/register
#
# スポナーの定義データ
#
# @within function asset:spawner/078/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:overworld,X:225,Y:62,Z:30}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [139,53]
# 一度に召喚する数 (int)
