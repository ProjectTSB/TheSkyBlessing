#> asset:spawner/024/register
#
# スポナーの定義データ
#
# @within function asset:spawner/024/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:overworld,X:-99,Y:17,Z:19}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [238,7]
# 一度に召喚する数 (int)
