...
# スポナーの定義データ
#
# @within function asset:spawner/104/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:overworld,X:104,Y:32,Z:137}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:80,Weight:4},{Id:217,Weight:1}]
# 一度に召喚する数 (int)
