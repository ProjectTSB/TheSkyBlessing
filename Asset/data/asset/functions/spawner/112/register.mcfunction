...
# スポナーの定義データ
#
# @within function asset:spawner/112/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:overworld,X:92,Y:31,Z:161}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:80,Weight:2},{Id:217,Weight:1}]
# 一度に召喚する数 (int)
