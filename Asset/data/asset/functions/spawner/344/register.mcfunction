...
# スポナーの定義データ
#
# @within function asset:spawner/344/


# 重複防止レジストリに登録
    data modify storage asset:spawner DPR append value {D:overworld,X:22,Y:161,Z:356}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [146,190]
# 一度に召喚する数 (int)
