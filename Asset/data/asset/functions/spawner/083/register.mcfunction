#> asset:spawner/083/register
#
# スポナーの定義データ
#
# @within function asset:spawner/083/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:overworld,X:-25,Y:41,Z:150}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [63,64]
# 一度に召喚する数 (int)
