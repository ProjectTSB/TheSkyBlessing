...
# スポナーの定義データ
#
# @within function asset:spawner/385/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:overworld,X:442,Y:34,Z:264}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [287,152]
# 一度に召喚する数 (int)
