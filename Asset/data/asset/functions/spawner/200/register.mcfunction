...
# スポナーの定義データ
#
# @within function asset:spawner/200/


# 重複防止レジストリに登録
    data modify storage asset:spawner DPR append value {D:overworld,X:90,Y:185,Z:-113}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [139]
# 一度に召喚する数 (int)
    data modify storage asset:spawner SpawnCount set value 8
