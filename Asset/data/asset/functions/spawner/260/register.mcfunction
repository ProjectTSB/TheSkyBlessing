...
# スポナーの定義データ
#
# @within function asset:spawner/260/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:overworld,X:111,Y:144,Z:-106}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:187,Weight:1},{Id:189,Weight:1},{Id:190,Weight:1}]
# 一度に召喚する数 (int)
