...
# スポナーの定義データ
# 
# @within function asset:spawner/222/


# 重複防止レジストリに登録
    data modify storage asset:spawner DPR append value {D:overworld,X:338,Y:85,Z:338}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [56,139]
# 一度に召喚する数 (int)
