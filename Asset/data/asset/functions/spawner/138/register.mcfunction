...
# スポナーの定義データ
#
# @within function asset:spawner/138/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:overworld,X:416,Y:160,Z:111}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:141,Weight:2},{Id:142,Weight:2},{Id:233,Weight:2}]
# 一度に召喚する数 (int)
