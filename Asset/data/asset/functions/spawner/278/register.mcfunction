...
# スポナーの定義データ
#
# @within function asset:spawner/278/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:overworld,X:36,Y:104,Z:-166}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [283]
# 一度に召喚する数 (int)
