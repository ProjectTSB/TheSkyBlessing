...
# スポナーの定義データ
#
# @within function asset:spawner/302/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:overworld,X:-52,Y:165,Z:-177}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [76,187]
# 一度に召喚する数 (int)
