...
# スポナーの定義データ
#
# @within function asset:spawner/380/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:overworld,X:338,Y:47,Z:266}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [287,187,204]
# 一度に召喚する数 (int)
