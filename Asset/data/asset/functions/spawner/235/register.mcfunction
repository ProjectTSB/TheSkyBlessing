...
# スポナーの定義データ
#
# @within function asset:spawner/235/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:overworld,X:169,Y:82,Z:36}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [15,37]
# 一度に召喚する数 (int)
