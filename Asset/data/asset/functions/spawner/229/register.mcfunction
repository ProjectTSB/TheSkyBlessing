...
# スポナーの定義データ
#
# @within function asset:spawner/229/


# 重複防止レジストリに登録
    data modify storage asset:spawner DPR append value {D:overworld,X:-40,Y:105,Z:-126}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [56,57]
# 一度に召喚する数 (int)
