#> asset:spawner/046/register
#
# スポナーの定義データ
#
# @within function asset:spawner/046/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:overworld,X:74,Y:7,Z:43}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [166,62]
# 一度に召喚する数 (int)
