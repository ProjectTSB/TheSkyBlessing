...
# スポナーの定義データ
#
# @within function asset:spawner/612/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:overworld,X:-130,Y:166,Z:-51}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [13,60]
# 一度に召喚する数 (int)
