#> asset:spawner/011/register
# 
# スポナーの定義データ
# 
# @within function asset:spawner/011/


# 重複防止レジストリに登録
    data modify storage asset:spawner DPR append value {D:overworld,X:-114,Y:12,Z:-114}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [124,7]
# 一度に召喚する数 (int)
