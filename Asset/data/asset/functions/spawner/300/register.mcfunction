...
# スポナーの定義データ
#
# @within function asset:spawner/300/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:overworld,X:-62,Y:175,Z:-196}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:76,Weight:2},{Id:84,Weight:2},{Id:114,Weight:1}]
# 一度に召喚する数 (int)
