#> asset:spawner/071/register
#
# スポナーの定義データ
#
# @within function asset:spawner/071/


# 重複防止レジストリに登録
    data modify storage asset:spawner DPR append value {D:overworld,X:144,Y:8,Z:133}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [13,107,37]
# 一度に召喚する数 (int)
