...
# スポナーの定義データ
#
# @within function asset:spawner/100/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:overworld,X:102,Y:21,Z:159}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:47,Weight:1},{Id:80,Weight:2}]
# 一度に召喚する数 (int)
