#> asset:spawner/092/register
#
# スポナーの定義データ
#
# @within function asset:spawner/092/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:overworld,X:-3,Y:7,Z:148}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [206,202]
# 一度に召喚する数 (int)
