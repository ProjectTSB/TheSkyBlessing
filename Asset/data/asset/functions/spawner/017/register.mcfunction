#> asset:spawner/017/register
#
# スポナーの定義データ
#
# @within function asset:spawner/017/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:overworld,X:-41,Y:38,Z:-121}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [166,63]
# 一度に召喚する数 (int)
