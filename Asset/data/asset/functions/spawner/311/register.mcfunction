...
# スポナーの定義データ
#
# @within function asset:spawner/311/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:overworld,X:-119,Y:139,Z:-61}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [23,245]
# 一度に召喚する数 (int)
