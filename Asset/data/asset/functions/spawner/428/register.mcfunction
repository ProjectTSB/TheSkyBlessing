...
# スポナーの定義データ
#
# @within function asset:spawner/428/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:overworld,X:-9,Y:156,Z:337}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:190,Weight:2},{Id:264,Weight:1}]
# 一度に召喚する数 (int)
