...
# スポナーの定義データ
# 
# @within function asset:spawner/195/


# 重複防止レジストリに登録
    data modify storage asset:spawner DPR append value {D:overworld,X:219,Y:149,Z:219}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [161]
# 一度に召喚する数 (int)
