...
# スポナーの定義データ
#
# @within function asset:spawner/592/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:the_end,X:172,Y:91,Z:1938}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [154,150]
# 一度に召喚する数 (int)
