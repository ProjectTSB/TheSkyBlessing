...
# スポナーの定義データ
#
# @within function asset:spawner/383/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:overworld,X:416,Y:42,Z:270}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [287,150]
# 一度に召喚する数 (int)
