...
# スポナーの定義データ
#
# @within function asset:spawner/129/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:overworld,X:390,Y:101,Z:33}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:141,Weight:2},{Id:142,Weight:2}]
# 一度に召喚する数 (int)
