#> asset:spawner/080/register
#
# スポナーの定義データ
#
# @within function asset:spawner/080/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:overworld,X:257,Y:62,Z:30}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [47,139]
# 一度に召喚する数 (int)
