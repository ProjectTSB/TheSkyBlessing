...
# スポナーの定義データ
#
# @within function asset:spawner/211/


# 重複防止レジストリに登録
    data modify storage asset:spawner DPR append value {D:overworld,X:31,Y:130,Z:-80}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [169,139]
# 一度に召喚する数 (int)
