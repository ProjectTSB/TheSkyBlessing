#> asset:spawner/012/register
#
# スポナーの定義データ
#
# @within function asset:spawner/012/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:overworld,X:-34,Y:37,Z:-92}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [26,7]
# 一度に召喚する数 (int)
