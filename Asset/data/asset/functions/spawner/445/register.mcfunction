...
# スポナーの定義データ
#
# @within function asset:spawner/445/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:overworld,X:-63,Y:166,Z:136}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:74,Weight:2},{Id:235,Weight:2},{Id:280,Weight:1}]
# 一度に召喚する数 (int)
