...
# スポナーの定義データ
#
# @within function asset:spawner/593/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:the_end,X:149,Y:92,Z:1902}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [153,153]
# 一度に召喚する数 (int)
