...
# スポナーの定義データ
#
# @within function asset:spawner/193/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:overworld,X:11,Y:13,Z:244}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [16]
# 一度に召喚する数 (int)
