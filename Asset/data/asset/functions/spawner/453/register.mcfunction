...
# スポナーの定義データ
#
# @within function asset:spawner/453/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:overworld,X:248,Y:168,Z:101}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [138,139]
# 一度に召喚する数 (int)
