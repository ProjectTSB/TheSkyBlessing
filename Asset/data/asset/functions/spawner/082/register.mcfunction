#> asset:spawner/082/register
#
# スポナーの定義データ
#
# @within function asset:spawner/082/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:overworld,X:1,Y:22,Z:122}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [29,205]
# 一度に召喚する数 (int)
