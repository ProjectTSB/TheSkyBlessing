#> asset:spawner/018/register
#
# スポナーの定義データ
#
# @within function asset:spawner/018/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:overworld,X:-99,Y:30,Z:23}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [21,63]
# 一度に召喚する数 (int)
