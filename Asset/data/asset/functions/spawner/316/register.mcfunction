...
# スポナーの定義データ
#
# @within function asset:spawner/316/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:overworld,X:-125,Y:166,Z:-62}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [7,124]
# 一度に召喚する数 (int)
