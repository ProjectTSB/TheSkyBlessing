...
# スポナーの定義データ
#
# @within function asset:spawner/174/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:overworld,X:190,Y:76,Z:266}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:190,Weight:4},{Id:210,Weight:1}]
# 一度に召喚する数 (int)
