...
# スポナーの定義データ
#
# @within function asset:spawner/419/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:overworld,X:67,Y:193,Z:405}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [{Id:181,Weight:4},{Id:264,Weight:2},{Id:210,Weight:1}]
# 一度に召喚する数 (int)
