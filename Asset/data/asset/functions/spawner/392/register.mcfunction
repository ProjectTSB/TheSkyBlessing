...
# スポナーの定義データ
#
# @within function asset:spawner/392/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:overworld,X:288,Y:29,Z:243}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [154,151]
# 一度に召喚する数 (int)
