#> asset:spawner/093/register
#
# スポナーの定義データ
#
# @within function asset:spawner/093/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:overworld,X:15,Y:7,Z:147}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [206,19]
# 一度に召喚する数 (int)
