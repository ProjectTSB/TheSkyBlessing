#> asset:spawner/065/register
#
# スポナーの定義データ
#
# @within function asset:spawner/065/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:overworld,X:121,Y:14,Z:42}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [63,29]
# 一度に召喚する数 (int)
