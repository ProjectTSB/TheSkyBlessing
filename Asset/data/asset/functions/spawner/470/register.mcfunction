...
# スポナーの定義データ
#
# @within function asset:spawner/470/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:overworld,X:-102,Y:224,Z:-46}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [124,56]
# 一度に召喚する数 (int)
