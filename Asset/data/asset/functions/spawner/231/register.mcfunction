...
# スポナーの定義データ
#
# @within function asset:spawner/231/


# 重複防止レジストリに登録
    data modify storage asset:spawner DPR append value {D:overworld,X:-32,Y:111,Z:-118}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [56,63]
# 一度に召喚する数 (int)
