#> asset:spawner/063/register
#
# スポナーの定義データ
#
# @within function asset:spawner/063/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:overworld,X:132,Y:16,Z:30}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [23,205]
# 一度に召喚する数 (int)
