#> asset:spawner/023/register
#
# スポナーの定義データ
#
# @within function asset:spawner/023/


# 重複防止レジストリへの登録
    data modify storage asset:spawner DPR append value {D:overworld,X:-95,Y:9,Z:28}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [239,7]
# 一度に召喚する数 (int)
