#> asset:spawner/091/register
# 
# スポナーの定義データ
# 
# @within function asset:spawner/091/


# 重複防止レジストリに登録
    data modify storage asset:spawner DPR append value {D:overworld,X:393,Y:142,Z:393}
...
# SpawnPotentials(int | int[] | ({ Weight: int, Id: int })[]) MobAssetのIDを指定する
    data modify storage asset:spawner SpawnPotentials set value [142,141]
# 一度に召喚する数 (int)
