#> asset:sacred_treasure/0999.gun_gun_gun_gun_gun_gun/trigger/attack
#
# 弾が着弾した時
#
# @within function asset:sacred_treasure/0999.gun_gun_gun_gun_gun_gun/trigger/bee

# 自身にタグ付与
    tag @s add RR.This

# 周囲2mから最寄りの敵にタグを付与
    tag @e[type=#lib:living,type=!player,tag=!RR.Bee,distance=..2,sort=nearest,limit=1] add RR.Target
...
# 補正functionを実行
    execute as @a if score @s UserID = @e[type=vex,tag=RR.This,limit=1] RR.UserID run function lib:damage/modifier
# 攻撃した対象に実行
    execute as @e[tag=RR.Target] run function lib:damage/
# リセット
