...
# 再帰処理 Attackerまでパーティクルを伸ばす
    ...
    execute if score @s HZ.Tick matches ..20 unless entity @e[type=#lib:living,tag=Attacker,distance=..0.5] facing entity @e[type=#lib:living,tag=Attacker,distance=..20] eyes positioned ^ ^ ^0.5 run function asset:sacred_treasure/0647.thorn_armor/9.exetend_thorn
