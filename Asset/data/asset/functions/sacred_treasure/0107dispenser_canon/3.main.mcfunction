...
# TNT召喚
    summon endermite ~ ~1.5 ~ {Silent:1b,FallDistance:99f,DeathTime:16s,Tags:["TNTCanon"],Passengers:[{id:"minecraft:tnt",Fuse:40s}],ActiveEffects:[{Id:14b,Amplifier:1b,Duration:2147483647}]}

# ベクトル取得用AEC召喚
    execute positioned 0.0 0.0 0.0 rotated as @s run summon area_effect_cloud ^ ^ ^2 {NoGravity:1b,Duration:1,Tags:[TNTCanon]}

# AECのベクトル取得
    execute positioned 0.0 0.0 0.0 as @e[type=area_effect_cloud,tag=TNTCanon,distance=..2] run data modify storage asset:temp 0107.Pos set from entity @s Pos

# TNTのmotionセット
    execute as @e[type=endermite,tag=TNTCanon,limit=1] run data modify entity @s Motion set from storage asset:temp 0107.Pos

# タグ消去
    tag @e[tag=TNTCanon,distance=..2] remove TNTCanon