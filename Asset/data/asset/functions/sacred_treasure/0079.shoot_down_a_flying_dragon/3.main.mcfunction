...
# ここから先は神器側の効果の処理を書く
    ...
    # 演出
        # Particle
            ...
            execute if score $AttackStrength Temporary matches 0 as @e[type=#lib:living,tag=Hit,distance=..10] at @s run particle dust 0.941 1 0.11 0.5 ~ ~2 ~ 0.02 5 0.02 0 150 force @a
            execute if score $AttackStrength Temporary matches 1 as @e[type=#lib:living,tag=Hit,distance=..10] at @s run particle dust 1 0.69 0.11 0.5 ~ ~2 ~ 0.02 5 0.02 0 150 force @a
            execute if score $AttackStrength Temporary matches 2 as @e[type=#lib:living,tag=Hit,distance=..10] at @s run particle dust 1 0.176 0.176 0.5 ~ ~2 ~ 0.02 5 0.02 0 150 force @a
            execute if score $AttackStrength Temporary matches 3 as @e[type=#lib:living,tag=Hit,distance=..10] at @s run particle dust 0.929 0.137 1 0.5 ~ ~2 ~ 0.02 5 0.02 0 150 force @a
        # Sound
            execute as @e[type=#lib:living,tag=Hit,distance=..10] at @s run playsound entity.lightning_bolt.thunder master @a ~ ~ ~ 10 2
            execute as @e[type=#lib:living,tag=Hit,distance=..10] at @s run playsound entity.lightning_bolt.thunder master @a ~ ~ ~ 10 2
            execute as @e[type=#lib:living,tag=Hit,distance=..10] at @s run playsound entity.lightning_bolt.thunder master @a ~ ~ ~ 10 2
    ...
    # リセット
        ...
        scoreboard players reset @e[type=#lib:living,distance=..10] Temporary
