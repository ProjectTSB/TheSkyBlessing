...
# 攻撃をさせる
    execute as @e[type=cod,tag=GY.Fish,scores={GY.FishCool=30}] at @s if entity @e[tag=Enemy,distance=..10] run function asset:sacred_treasure/0610.call_fish/trigger/3.2.attack
...
# 再帰処理
    execute if entity @e[type=cod,tag=GY.Fish,limit=1] run schedule function asset:sacred_treasure/0610.call_fish/trigger/3.1.tick 1t replace