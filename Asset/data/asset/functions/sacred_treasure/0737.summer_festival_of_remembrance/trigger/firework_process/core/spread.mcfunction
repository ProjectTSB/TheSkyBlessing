...
# マーカーを拡散させ、その位置で花火召喚処理を実行する
    ...
    data modify storage lib: Argument.Bounds set value [[30d,30d],[0d,0d],[30d,30d]]
    execute as @e[type=marker,tag=SpreadMarker,distance=..0.01,limit=1] at @s run function lib:spread_entity/
    execute at @e[type=marker,tag=SpreadMarker,distance=..50,sort=nearest,limit=1] positioned ~ ~1 ~ run function asset:sacred_treasure/0737.summer_festival_of_remembrance/trigger/firework_process/summon/
