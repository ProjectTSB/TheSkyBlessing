#> asset:sacred_treasure/0648.thorn_leggings/_.give
#
# 神器の作成部 ここでID等を定義する
#
# @user
...
# 神器のID (int) スプレッドシートの値を入れる
    data modify storage asset:sacred_treasure ID set value 648
# 神器のベースアイテム
    data modify storage asset:sacred_treasure Item set value "minecraft:leather_leggings"
# 神器の名前 (TextComponentString)
    data modify storage asset:sacred_treasure Name set value '{"text":"ソーンレギンス","color":"#1D5923"}'
# 神器の説明文 (TextComponentString[])
    data modify storage asset:sacred_treasure Lore set value ['[{"text":"水耐性+5% ","color":"aqua"},{"text":"雷耐性+5%","color":"yellow"}]','{"text":"フルセット効果でMPを10消費する","color":"white"}']
...
# 神器の発動条件 (TextComponentString) (オプション)
    data modify storage asset:sacred_treasure Condition set value '{"text":"荊の鎧を全て纒いし時、血吸いの荊が目を覚ます","color":"#1D5923"}'
...
# カスタムNBT (NBTCompound) 追加で指定したいNBT (オプション)
    data modify storage asset:sacred_treasure CustomNBT set value {HideFlags:68,display:{color:2454317},Unbreakable:1b,Enchantments:[{id:"thorns",lvl:5},{id:"protection",lvl:3}],AttributeModifiers:[{AttributeName:"generic.armor",Name:"generic.armor",Amount:5,Operation:0,UUID:[I;1,1,648,4],Slot:"legs"}]}
