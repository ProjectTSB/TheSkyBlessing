...
# パーティクル
    execute positioned ~ ~0.8 ~ run particle minecraft:electric_spark ^ ^ ^-2 0 0 0 0.5 5

# スコア加算
    scoreboard players add @s R4.Tick 1

# 後からモデル装着
    execute if entity @s[scores={R4.Tick=5..7}] run particle minecraft:end_rod ~ ~1 ~ 0 0 0 0.1 3
...
# キル
    execute if entity @s[scores={R4.Tick=20..}] positioned ~ ~0.8 ~ run particle minecraft:electric_spark ^ ^ ^-2 0 0 0 0.7 20
