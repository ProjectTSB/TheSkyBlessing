...
# ここから先は神器側の効果の処理を書く
    ...
    # 演出
        ...
        execute at @e[type=#lib:living,type=!player,tag=Victim,tag=Hit,distance=..100,limit=1] run playsound entity.lightning_bolt.thunder player @a[distance=..100] ~ ~ ~ 10000000000 2 1
        execute at @e[type=#lib:living,type=!player,tag=Victim,tag=Hit,distance=..100,limit=1] run playsound entity.lightning_bolt.thunder player @a[distance=..100] ~ ~ ~ 10000000000 2 1
