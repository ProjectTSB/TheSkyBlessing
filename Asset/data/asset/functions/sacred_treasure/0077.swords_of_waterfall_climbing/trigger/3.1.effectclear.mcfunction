...
#   asset:sacred_treasure/0077.swords_of_waterfall_climbing/trigger/3.1.effectclear
#   asset:sacred_treasure/0077.swords_of_waterfall_climbing/trigger/rejoin_process
...
# 落下量を検知
    execute as @a[tag=25.EffectClear] run function asset:sacred_treasure/0077.swords_of_waterfall_climbing/trigger/3.3.falldistance
...
# 着地時の処理
    ...
    # 着地時に周囲にダメージ与える
        execute as @a[tag=25.EffectClear,tag=25.Landing] if data storage api: {OnGround:1b} at @s run function asset:sacred_treasure/0077.swords_of_waterfall_climbing/trigger/3.3.landing_attack
    execute if data storage api: {OnGround:1b} run tag @a[tag=25.EffectClear] remove 25.EffectClear

# 着地していない場合、次tickも実行
    execute if entity @a[tag=25.EffectClear,limit=1] run schedule function asset:sacred_treasure/0077.swords_of_waterfall_climbing/trigger/3.1.effectclear 1t replace