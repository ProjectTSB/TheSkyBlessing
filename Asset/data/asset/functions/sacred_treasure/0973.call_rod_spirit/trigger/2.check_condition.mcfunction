...
# 既に召喚済みなら使えない。ファミリアと違って召喚するのは一匹だが、合わせておくと都合がいいし、Lintを通すのも楽
    execute at @e[type=armor_stand,tag=R1.Spirit] if score @s UserID = @e[type=armor_stand,tag=R1.Spirit,sort=nearest,limit=1] R1.UserID run tag @s add R1.CannotUse
    execute if entity @s[tag=R1.CannotUse] run tag @s remove CanUsed
    execute if entity @s[tag=R1.CannotUse] run function asset:sacred_treasure/0973.call_rod_spirit/trigger/3.cannot_use
