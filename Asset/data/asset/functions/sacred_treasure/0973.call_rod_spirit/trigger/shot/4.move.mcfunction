...
# 再帰カウントが0なら弾速からセット
    execute unless entity @s[scores={R1.Recursive=1..,R1.Range=1..}] run scoreboard players operation @s R1.Recursive = @s R1.Speed

# 前進
    execute if entity @s[scores={R1.Recursive=1..,R1.Range=1..}] run tp @s ^ ^ ^0.1 ~ ~

# スコア減算
    scoreboard players remove @s R1.Recursive 1
...
# 再帰
    execute if entity @s[scores={R1.Recursive=1..,R1.Range=1..}] at @s run function asset:sacred_treasure/0973.call_rod_spirit/trigger/shot/4.move
