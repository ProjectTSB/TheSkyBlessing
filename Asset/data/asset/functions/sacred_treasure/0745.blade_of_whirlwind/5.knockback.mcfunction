...
# ノクバ耐性が0でない限り100から数値を引く
    execute if score $MobKnockbackResist Temporary matches 1..99 store result score $MobKnockbackResist Temporary run scoreboard players operation $100 Const -= $MobKnockbackResist Temporary

# 移動速度上昇ないとバニラと同じくらいで悲しいので5ブロック分
    scoreboard players add $VectorMagnitude Lib 50
