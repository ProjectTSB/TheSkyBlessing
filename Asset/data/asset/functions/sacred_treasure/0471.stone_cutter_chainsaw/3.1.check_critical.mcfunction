...
# 3.5マスまで再帰
    execute unless entity @e[type=#lib:living,type=!player,tag=D3.Critical,tag=!Uninterferable,dx=0] if score $7n.Count Temporary matches ..35 positioned ^ ^ ^0.1 run function asset:sacred_treasure/0471.stone_cutter_chainsaw/3.1.check_critical