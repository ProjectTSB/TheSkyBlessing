...
# エンチェスに変換
    execute as @p[y_rotation=45..134] at @s run setblock ~ ~ ~ ender_chest[facing=east]
    execute as @p[y_rotation=135..224] at @s run setblock ~ ~ ~ ender_chest[facing=south]
    execute as @p[y_rotation=225..314] at @s run setblock ~ ~ ~ ender_chest[facing=west]
    execute as @p[y_rotation=315..44] at @s run setblock ~ ~ ~ ender_chest[facing=north]
...
# 効果音
    ...
    playsound minecraft:block.note_block.banjo neutral @a ~ ~-2 ~ 1 0.8
