...
# テラショットにコマンド実行
    execute as @e[type=armor_stand,tag=5U.Shot] at @s run function asset:sacred_treasure/0210.terra_blade/trigger/3.3.terra_shot_main

# ビームが存在していれば、このfunctionをループさせるよ
    execute if entity @e[type=armor_stand,tag=5U.Shot,limit=1] run schedule function asset:sacred_treasure/0210.terra_blade/trigger/3.1.terra_shot_tick 1t