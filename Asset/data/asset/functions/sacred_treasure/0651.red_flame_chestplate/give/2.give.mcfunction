#> asset:sacred_treasure/0651.red_flame_chestplate/give/2.give
#
# 神器の作成部 ここでID等を定義する
#
# @user
# @within function asset:sacred_treasure/0651.red_flame_chestplate/give/1.trigger
...
# 神器の名前 (TextComponentString)
    data modify storage asset:sacred_treasure Name set value '{"text":"紅焔","color":"red"}'
# 神器の説明文 (TextComponentString[])
    data modify storage asset:sacred_treasure Lore set value ['{"text":"灼熱の如き熱を秘めた鎧","color":"yellow"}','{"text":"その全てを揃えた時、紅焔の如く敵を焼き尽くす"}']
...
# カスタムNBT (NBTCompound) 追加で指定したいNBT (オプション)
    data modify storage asset:sacred_treasure CustomNBT set value {HideFlags:68,display:{color:13569550},Unbreakable:1b,AttributeModifiers:[]}
