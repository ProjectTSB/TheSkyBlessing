...
# 周囲10M以内の敵全てのHPを取得
    execute as @e[tag=Enemy,tag=!Uninterferable,distance=..10] store result score @s Temporary run data get storage api: Health 1
# 周囲10M以内のHPが20以下のエンティティ1体を絞り込み、ターゲットタグを付与
    execute as @e[tag=Enemy,tag=!Uninterferable,scores={Temporary=..20},distance=..10,limit=1] run tag @s add 7k.Target
