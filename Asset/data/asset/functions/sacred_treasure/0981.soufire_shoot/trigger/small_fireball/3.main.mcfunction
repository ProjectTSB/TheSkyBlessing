...
# 移動 カーブをfunction内で行うと曲がりすぎるのでここで書く
    execute facing entity @e[tag=Enemy,tag=!Uninterferable,tag=!R9.NotTarget,distance=..20,sort=nearest,limit=1] eyes positioned ^ ^ ^-50 rotated as @s positioned ^ ^ ^-400 facing entity @s eyes positioned as @s run function asset:sacred_treasure/0981.soufire_shoot/trigger/small_fireball/4.move
    execute unless entity @e[tag=Enemy,tag=!Uninterferable,tag=!R9.NotTarget,distance=..20,sort=nearest,limit=1] run function asset:sacred_treasure/0981.soufire_shoot/trigger/small_fireball/4.move
