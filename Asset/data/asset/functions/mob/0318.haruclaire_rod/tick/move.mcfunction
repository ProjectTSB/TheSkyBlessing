...
# プレイヤーの衝突判定
    execute if score @s 8U.CoolTime matches ..0 positioned ~ ~0.9 ~ if entity @a[distance=..1.5] run function asset:mob/0318.haruclaire_rod/tick/damage_move
...
# 以下キル処理
    # 壁の衝突判定
        execute positioned ~ ~0.9 ~ unless block ~ ~ ~ #lib:no_collision run function asset:mob/0318.haruclaire_rod/tick/damage_break
    # 生存時間
        execute if entity @s[scores={8U.Range=..0}] positioned ~ ~0.9 ~ run function asset:mob/0318.haruclaire_rod/tick/kill