...
# プレイヤー用のダメージファンクション
    execute as @a[distance=..30] at @s run function asset:mob/0216.gray_guardian/tick/4.skill_impact/5.damage_player
# 上空へ飛ばす
    execute as @e[type=#lib:living,type=!player,tag=Friend,tag=!Uninterferable,nbt={OnGround:1b},distance=..30] at @s run summon area_effect_cloud ~ ~ ~ {Duration:14,Age:4,Effects:[{Id:25b,Amplifier:80b,Duration:3,ShowParticles:0b},{Id:11b,Amplifier:127b,Duration:1,ShowParticles:0b},{Id:7b,Amplifier:0b,Duration:1,ShowParticles:0b}]}
# リセット
