...
# 発砲(発砲が終わったら終了へ行く)
    execute if score @s 60.Tick matches 30..70 positioned ^-0.8 ^ ^2.5 positioned ~ ~1.2 ~ rotated ~-8 ~10 run function asset:mob/0216.gray_guardian/tick/2.skill_bow/3.shot
    execute if score @s 60.Tick matches 30..70 run tag @s remove Landing
# プレイヤーを追尾
    execute if score @s 60.Tick matches 30.. facing entity @p[gamemode=!spectator,distance=..30] eyes positioned ^ ^ ^-100 rotated as @s positioned ^ ^ ^-200 facing entity @s feet positioned as @s positioned ^ ^ ^ run function asset:mob/0216.gray_guardian/tick/move/tereport
# 発射モデル
