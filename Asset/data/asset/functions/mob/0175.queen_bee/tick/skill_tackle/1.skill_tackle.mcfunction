...
# 突進の再開
    execute if score @s 4V.Tick matches 55..64 at @s run function asset:mob/0175.queen_bee/tick/skill_tackle/2.skill_tackle_tp
    execute if entity @s[tag=!4V.SkillTackleDamageHit] if score @s 4V.Tick matches 65..84 at @s run function asset:mob/0175.queen_bee/tick/skill_tackle/3.skill_tackle_damage
    execute if score @s 4V.Tick matches 55..64 at @s run function asset:mob/0175.queen_bee/tick/skill_tackle/2.skill_tackle_tp
    execute if entity @s[tag=!4V.SkillTackleDamageHit] if score @s 4V.Tick matches 65..84 at @s run function asset:mob/0175.queen_bee/tick/skill_tackle/3.skill_tackle_damage
# 攻撃判定のタグを消す
    execute if score @s 4V.Tick matches 65 run tag @s remove 4V.SkillTackleDamageHit

# 終了処理
# モデル戻す
    execute if score @s 4V.Tick matches 65 run item replace entity @e[type=armor_stand,tag=4V.ArmorStand,distance=..0.01,sort=nearest,limit=1] armor.head with stone{CustomModelData:20052}
# teleportして少し待つ
    execute if score @s 4V.Tick matches 65 run tag @s remove 4V.SkillTackle
