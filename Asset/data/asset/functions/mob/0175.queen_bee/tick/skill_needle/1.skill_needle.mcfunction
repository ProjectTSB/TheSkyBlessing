...
# 発砲準備
    execute if score @s 4V.Tick matches 20..25 positioned ~ ~-1.5 ~ positioned ^ ^ ^1 run function asset:mob/0175.queen_bee/tick/skill_needle/2.skill_needle_ready

# 確率で繰り返す
    execute if score @s 4V.Tick matches 30 if predicate lib:random_pass_per/50 run scoreboard players set @s 4V.Tick 0
