...
# 交互に放つ
    ...
    execute if score @s 1J.Tick matches 35 anchored eyes positioned ^-1 ^-0.5 ^1 run function asset:mob/0055.hetukedah/tick/skill/otete_beam/shoot

# 最後にデカいのを撃つ
    execute if score @s 1J.Tick matches 40 anchored eyes positioned ^ ^-0.5 ^1 run function asset:mob/0055.hetukedah/tick/skill/otete_beam/big_shoot
