...
# 最初に実行するの
    execute if score @s 7A.Tick matches 0 run function asset:mob/0262.frestchika/tick/2.melee/2.first_tick
...
# 突進でプレイヤーが見つかってスコアが飛んだ先
    ...
    execute if score @s 7A.Tick matches 1004 facing entity @p feet run function asset:mob/0262.frestchika/tick/move/tereport
    ...
    execute if score @s 7A.Tick matches 1006 at @s rotated ~-40 ~ run function asset:mob/0262.frestchika/tick/move/tereport
