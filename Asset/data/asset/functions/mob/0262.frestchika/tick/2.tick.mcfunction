...
# オーバーヒートしていない限り常時耐性1がつく
    effect give @s[tag=!7A.SkillOverHeat] resistance 1 0 true

# スコアを増やす
    scoreboard players add @s 7A.Tick 1

# プレイヤーを見る
    execute if score @s 7A.Tick matches 0 at @s facing entity @p feet run function asset:mob/0262.frestchika/tick/move/tereport
