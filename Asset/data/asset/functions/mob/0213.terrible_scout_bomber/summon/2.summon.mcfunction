#> asset:mob/0213.terrible_scout_bomber/summon/2.summon
#
# Mobの召喚時の処理
#
...
# 名前 (TextComponentString) (オプション)
    data modify storage asset:mob Name set value '{"text":"Terrible Scout Bomber","color":"#FFFFFF","bold":true,"underlined":true}'
# 武器
...
# 体力 (double) (オプション)
    data modify storage asset:mob Health set value 700d
# 攻撃力 (double) (オプション)
    data modify storage asset:mob AttackDamage set value 0d
# 防御力 (double) (オプション) // 被ダメージがある程度大きい場合1ptにつき0.8%カット、小さい場合1ptにつき約4%カット 20pt以上は頭打ち
    data modify storage asset:mob Defense set value 20d
# 特殊防御力 (double) (オプション) // 4pointにつきダメージを大きく減らす
    data modify storage asset:mob SpecialDefense set value 16d
# 移動速度 (double) (オプション)
...
# 属性倍率 // 1.0fで100% 最低でも25%は軽減されずに入る
    # 物理倍率 (float) (オプション)
        data modify storage asset:mob Resist.Physical set value 0.66f
    # 魔法倍率 (float) (オプション)
        data modify storage asset:mob Resist.Magic set value 0.9f
    # 火倍率 (float) (オプション)
        data modify storage asset:mob Resist.Fire set value 1f
    # 水倍率 (float) (オプション)
