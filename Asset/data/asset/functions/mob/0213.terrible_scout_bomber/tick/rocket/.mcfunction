...
# move to front of target
    execute if score $PhaseTime Temporary matches 1 as @a if score @s UserID = @e[tag=this,distance=..0.1,limit=1] 5X.TargetId at @s rotated ~ 0 positioned ^ ^ ^20 positioned ~ ~5 ~ facing entity @s eyes store result score $5X.TpSuccess Temporary run tp @e[tag=this,limit=1] ~ ~ ~ ~ ~
...
# move forward
    execute if score $PhaseTime Temporary matches 2..32 at @s run tp @s ^ ^0.5 ^0.1 ~ 90
...
# move to climb phase
    execute if score $PhaseTime Temporary matches 33.. run scoreboard players set @s 5X.ActionPhase 7
    execute if score $PhaseTime Temporary matches 33.. store result score @s 5X.PhStartTime run time query gametime
