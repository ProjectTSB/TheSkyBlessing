...
# 頭のポーションの位置が当たり判定のあるブロックなら実行
    execute if entity @s[tag=!2X.AlreadyBroken] positioned ~ ~2 ~ unless block ~ ~ ~ #lib:no_collision run function asset:mob/0105.potion_projectile/tick/4.break_potion

# 5.break_potionが実行された位置でメインの演出
    execute if entity @s[tag=2X.AlreadyBroken] positioned ~ ~2.3 ~ run function asset:mob/0105.potion_projectile/tick/5.element_branch
