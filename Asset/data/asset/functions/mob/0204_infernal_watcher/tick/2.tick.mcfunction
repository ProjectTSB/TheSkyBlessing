...
# スコアによって速度が変わる、最高速のときはダメージを受けても止まらない
    execute if entity @s[scores={5O.MoveTime=..59,5O.HurtTime=0}] facing entity @p eyes positioned ^ ^ ^-100 rotated as @s positioned ^ ^ ^-800 facing entity @s eyes positioned as @s run tp @s ^ ^ ^0.1 ~ ~
    execute if entity @s[scores={5O.MoveTime=60..}] facing entity @p eyes positioned ^ ^ ^-100 rotated as @s positioned ^ ^ ^-800 facing entity @s eyes positioned as @s run tp @s ^ ^ ^0.5 ~ ~
...
# 接地で上を向く
    execute positioned ~ ~1.68 ~ unless block ~ ~-0.1 ~ #lib:no_collision at @s run tp @s ~ ~ ~ ~ ~-35
...
# クールタイム中じゃないなら接触時に攻撃
    execute if entity @s[scores={5O.MoveTime=..59}] unless score @s 5O.AttackCT matches 1.. positioned ~-0.5 ~-0.5 ~-0.5 if entity @p[gamemode=!creative,gamemode=!spectator,dx=0] run function asset:mob/0204_infernal_watcher/tick/event/attack
    execute if entity @s[scores={5O.MoveTime=60..}] unless score @s 5O.AttackCT matches 1.. positioned ~-0.5 ~-0.5 ~-0.5 if entity @p[gamemode=!creative,gamemode=!spectator,dx=0] run function asset:mob/0204_infernal_watcher/tick/event/attack_strong
