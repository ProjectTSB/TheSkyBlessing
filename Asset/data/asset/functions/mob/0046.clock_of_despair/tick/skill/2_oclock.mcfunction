...
# ビーム
    ...
    execute if score @s 1A.LifeTime matches 401 run tp @e[type=area_effect_cloud,tag=1A.SkillBeam,distance=..100] ~ ~10 ~ 180 90
# ビーム追尾
    execute if score @s 1A.LifeTime matches 421..520 as @e[type=area_effect_cloud,tag=1A.SkillBeam,distance=..100] at @s run execute if entity @p[distance=..32] facing entity @p eyes positioned ^ ^ ^-100 rotated as @s positioned ^ ^ ^-800 facing entity @s feet positioned as @s run tp @s ^ ^ ^ ~ ~
