...
# 床の時
    ...
    # ブロック終わったなら壁下りタグ
        execute as @s[tag=7L.FaceFloor,tag=7L.FaceCheck] if block ^ ^-1 ^1 #lib:no_collision run function asset:mob/0273.hell_blade/move/face/wall/down

# 壁で上向きの時
    # ぶつかったなら天井タグ
        execute as @s[tag=7L.FaceWallUp,tag=7L.FaceCheck] unless block ~ ~1 ~ #lib:no_collision run function asset:mob/0273.hell_blade/move/face/ceiling

    # ブロック終わったなら床タグ
        execute as @s[tag=7L.FaceWallUp,tag=7L.FaceCheck] if block ^ ^1 ^1 #lib:no_collision run function asset:mob/0273.hell_blade/move/face/floor
