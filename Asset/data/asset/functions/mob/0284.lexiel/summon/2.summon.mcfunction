...
# 元となるMobを召喚する
    summon wither_skeleton ~ ~ ~ {Tags:["MobInit","AlwaysInvisible","7W.Angel"],NoAI:1b,Silent:1b,PersistenceRequired:1b,DeathLootTable:"asset:mob/death/0284.lexiel"}
# ID (int)
...
# 位置をあわせる
    execute as @e[type=wither_skeleton,tag=MobInit,distance=..0.01] at @s run tp @e[type=armor_stand,tag=7W.ArmorStandThis,distance=..0.01] @s
# タグを消す
    tag @e[type=armor_stand,tag=7W.ArmorStandThis,distance=..0.01] remove 7W.ArmorStandThis
