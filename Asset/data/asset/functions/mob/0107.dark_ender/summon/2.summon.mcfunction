#> asset:mob/0107.dark_ender/summon/2.summon
#
# Mobの召喚時の処理
#
# @within function asset:mob/0107.dark_ender/summon/1.trigger

# 元となるMobを召喚する
    summon enderman ~ ~ ~ {Tags:["MobInit","AlwaysInvisible"],DeathLootTable:"asset:mob/death/0107.dark_ender"}
# ID (int)
    data modify storage asset:mob ID set value 107
# Type (string) Wikiを参照
    data modify storage asset:mob Type set value "Enemy"
# 干渉可能か否か (boolean)
    data modify storage asset:mob Interferable set value true
# 名前 (TextComponentString) (オプション)
    data modify storage asset:mob Name set value '{"text":"ダークエンダー","color":"dark_purple"}'
# 武器
...
# MobInitタグ持ちを対象にして召喚関数呼び出し
    execute as @e[type=enderman,tag=MobInit,distance=..0.01] run function asset:mob/common/summon