...
# テレポート
# アマスタにタグを付与
    execute if score @s M.Tick matches 15 run tag @e[type=armor_stand,tag=M.ArmorStand,distance=..0.01,sort=nearest,limit=1] add M.ArmorStandThis
# 前進
    execute if score @s M.Tick matches 15 run tp ^ ^ ^4
# アマスタを持ってきてタグを消す
    execute if score @s M.Tick matches 15 run tp @e[type=armor_stand,tag=M.ArmorStandThis,sort=nearest,limit=1] @s
    execute if score @s M.Tick matches 15 run tag @e[type=armor_stand,tag=M.ArmorStandThis,sort=nearest,limit=1] remove M.ArmorStandThis
...
# ダメージ発生
    execute if score @s M.Tick matches 15 positioned ^ ^ ^1 run function asset:mob/0022.red_knight/tick/skill_sword/2.skill_sword_damage
...
# テレポート
# アマスタにタグを付与
    execute if score @s M.Tick matches 20 run tag @e[type=armor_stand,tag=M.ArmorStand,distance=..0.01,sort=nearest,limit=1] add M.ArmorStandThis
# 前進
    execute if score @s M.Tick matches 20 run tp ^ ^ ^4
# アマスタを持ってきてタグを消す
    execute if score @s M.Tick matches 20 run tp @e[type=armor_stand,tag=M.ArmorStandThis,sort=nearest,limit=1] @s
    execute if score @s M.Tick matches 20 run tag @e[type=armor_stand,tag=M.ArmorStandThis,sort=nearest,limit=1] remove M.ArmorStandThis
...
# ダメージ発生
    execute if score @s M.Tick matches 20 positioned ^ ^ ^5 run function asset:mob/0022.red_knight/tick/skill_sword/2.skill_sword_damage


# 共通リセットへ
    execute if score @s M.Tick matches 35 run function asset:mob/0022.red_knight/tick/reset