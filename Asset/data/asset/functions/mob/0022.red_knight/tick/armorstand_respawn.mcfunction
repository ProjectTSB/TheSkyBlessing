...
# どっかいったアマスタを殺す
    execute as @e[type=armor_stand,tag=M.ArmorStand] at @s unless entity @e[type=wither_skeleton,tag=M.Angel,distance=0] run kill @s
# 見た目用のアマスタを召喚
...
# スキル発動中に移動した場合困るので消す
    ...
    tag @s remove M.SkillMagic
    tag @s remove M.SkillExplode
    tag @s remove M.SkillExplodeParticle
    tag @s remove M.SkillThunder
