...
# 演出
    execute if entity @a[gamemode=!creative,gamemode=!spectator,distance=..1] at @a[gamemode=!creative,gamemode=!spectator,distance=..1] run playsound entity.lightning_bolt.thunder hostile @a ~ ~ ~ 1 2

# もし不発だったらスコアを勧めちゃって終わり
    execute unless entity @a[gamemode=!creative,gamemode=!spectator,distance=..1] run scoreboard players set @s M.Tick 45