...
# ブロックだったらパーティクル投影
    execute unless block ~ ~ ~ #lib:no_collision positioned ~ ~1 ~ run function asset:mob/0022.red_knight/tick/skills/hyper_explosion/3.circle_particle

# 奈落だったり、下が遠すぎたらそのまま実行
    execute unless entity @s[distance=..16] at @r run function asset:mob/0022.red_knight/tick/skills/hyper_explosion/3.circle_particle
