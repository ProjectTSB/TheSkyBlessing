#> asset:mob/0238.poltergeist_armor/summon/2.summon
#
# Mobの召喚時の処理
#
# @within function asset:mob/0238.poltergeist_armor/summon/1.trigger

# 元となるMobを召喚する
    summon zombie ~ ~ ~ {Silent:1b,Tags:["MobInit","AlwaysInvisible","AntiBurn"],DeathLootTable:"asset:mob/death/0238.poltergeist_armor"}
# ID (int)
    data modify storage asset:mob ID set value 238
# Type (string) Wikiを参照
...
# 防具
    ...
    # 胴 (Compound(Item)) (オプション)
     data modify storage asset:mob Armor.Chest set value {id:"minecraft:leather_chestplate",Count:1b}
    # 脚 (Compound(Item)) (オプション)
        data modify storage asset:mob Armor.Legs set value {id:"minecraft:leather_leggings",Count:1b}
...
# 属性倍率 // 1.0fで100% 最低でも25%は軽減されずに入る
    # 物理倍率 (float) (オプション)
        data modify storage asset:mob Resist.Physical set value 0.50
    # 魔法倍率 (float) (オプション)
