#> asset:mob/0242.poltergeist_subspecies_bow/summon/2.summon
#
# Mobの召喚時の処理
#
# @within function asset:mob/0242.poltergeist_subspecies_bow/summon/1.trigger

# 元となるMobを召喚する
    summon wither_skeleton ~ ~ ~ {Tags:["MobInit","AlwaysInvisible"],DeathLootTable:"asset:mob/death/0242.poltergeist_subspecies_bow",Silent:1b}
# ID (int)
    data modify storage asset:mob ID set value 242
# Type (string) Wikiを参照
...
# 武器
    ...
    # オフハンド (Compound(Item)) (オプション)
        data modify storage asset:mob Weapon.Offhand set value {id:"minecraft:shield",Count:1b}
...
# 属性倍率 // 1.0fで100% 最低でも25%は軽減されずに入る
    ...
    # 魔法倍率 (float) (オプション)
        data modify storage asset:mob Resist.Magic set value 50
    # 火倍率 (float) (オプション)
