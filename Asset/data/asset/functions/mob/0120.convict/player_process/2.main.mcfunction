...
# 音
    scoreboard players operation $Temp Temporary = @s 3C.Time
...
# スコア
    scoreboard players remove @s 3C.Time 1

# ループ
    execute if entity @s[scores={3C.Time=0..}] run schedule function asset:mob/0120.convict/player_process/1.loop 1t replace

# 効果終了処理
    execute if entity @s[scores={3C.Time=..0}] run function asset:mob/0120.convict/player_process/3.reset