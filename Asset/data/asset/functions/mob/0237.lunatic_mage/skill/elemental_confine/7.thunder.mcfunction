...
# 一定間隔で実行
    ...
    scoreboard players operation $Temp Temporary %= $80 Const
...
# 30のときも3つ落とす
    execute if score $Temp Temporary matches 40 rotated ~60 ~ positioned ^ ^ ^2.5 run function asset:mob/0237.lunatic_mage/magic_summon/thunder
    execute if score $Temp Temporary matches 40 rotated ~180 ~ positioned ^ ^ ^2.5 run function asset:mob/0237.lunatic_mage/magic_summon/thunder
    execute if score $Temp Temporary matches 40 rotated ~300 ~ positioned ^ ^ ^2.5 run function asset:mob/0237.lunatic_mage/magic_summon/thunder
