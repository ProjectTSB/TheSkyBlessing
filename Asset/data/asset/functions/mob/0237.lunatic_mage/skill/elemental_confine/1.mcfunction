...
# 召喚された地点にランダムなプレイヤー一人と、水の檻範囲内にいたプレイヤーと共にワープ
    ...
    execute if entity @s[scores={6L.Tick=20}] positioned as @e[type=marker,tag=6L.SpawnPoint,distance=..100,sort=nearest,limit=1] run tp @a[distance=..9] ~ ~ ~

# アクティブ中に実行するコマンド
    execute if entity @s[scores={6L.Tick=21..450}] run function asset:mob/0237.lunatic_mage/skill/elemental_confine/2.in_activation
...
# 雷を落とす
    execute if entity @s[scores={6L.Tick=175..450}] run function asset:mob/0237.lunatic_mage/skill/elemental_confine/10.thunder
