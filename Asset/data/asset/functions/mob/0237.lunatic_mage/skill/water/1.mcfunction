...
# 魔法召喚
    execute if entity @s[scores={6L.Tick=21}] at @a[gamemode=!spectator,distance=..50,sort=random,limit=5] rotated ~ 0 positioned ~ ~0.3 ~ run function asset:mob/0237.lunatic_mage/magic_summon/water
    execute if entity @s[scores={6L.Tick=41}] at @a[gamemode=!spectator,distance=..50,sort=random,limit=5] rotated ~ 0 positioned ~ ~0.3 ~ run function asset:mob/0237.lunatic_mage/magic_summon/water
