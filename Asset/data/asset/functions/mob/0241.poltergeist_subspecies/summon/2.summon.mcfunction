#> asset:mob/0241.poltergeist_subspecies/summon/2.summon
#
# Mobの召喚時の処理
#
# @within function asset:mob/0241.poltergeist_subspecies/summon/1.trigger

# 元となるMobを召喚する
    summon wither_skeleton ~ ~ ~ {Tags:["MobInit","AlwaysInvisible"],DeathLootTable:"asset:mob/death/0241.poltergeist_subspecies",Silent:1b}
# ID (int)
    data modify storage asset:mob ID set value 241
# Type (string) Wikiを参照
...
# 属性倍率 // 1.0fで100% 最低でも25%は軽減されずに入る
    # 物理倍率 (float) (オプション)
        data modify storage asset:mob Resist.Physical set value 50
    # 魔法倍率 (float) (オプション)
