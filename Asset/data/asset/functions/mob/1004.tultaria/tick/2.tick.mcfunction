...
# その後発動するスキル
# プレイヤーが周囲にいたらスキル選択
    execute if score @s RW.Tick matches 0 if entity @s[tag=!RW.SkillFixed] if entity @p[gamemode=!spectator,distance=..100] run function asset:mob/1004.tultaria/tick/3.skill_select
    execute if score @s RW.Tick matches 0 if entity @s[tag=RW.SkillFixed] if entity @p[gamemode=!spectator,distance=..100] run tag @s remove RW.SkillFixed
