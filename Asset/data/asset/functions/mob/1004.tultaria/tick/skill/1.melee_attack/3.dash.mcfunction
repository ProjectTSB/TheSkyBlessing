...
# ポーズ
    data modify entity @e[type=armor_stand,tag=RW.ModelBody,distance=..0.01,sort=nearest,limit=1] Pose.LeftArm set value [20f,0f,-75f]
    data modify entity @e[type=armor_stand,tag=RW.ModelBody,distance=..0.01,sort=nearest,limit=1] Pose.RightArm set value [-35f,0f,75f]
...
# 体の向き
    execute at @s run tp @e[type=armor_stand,tag=RW.ModelBody,distance=..0.01,sort=nearest,limit=1] ~ ~ ~ ~-45 ~
...
# 頭のモデル、向き
    ...
    execute as @e[type=armor_stand,tag=RW.ModelHead,distance=..0.01,sort=nearest,limit=1] run tp @s ~ ~ ~ ~ ~
