...
# 腕を変える
    execute if score @s RW.Tick matches 20 if entity @s[tag=RW.HPless75per] run data modify entity @e[type=armor_stand,tag=RW.ArmorStand,distance=..0.01,sort=nearest,limit=1] Pose.RightArm set value [215f,0f,94f]

# HPが減っていたら攻撃
    execute if score @s RW.Tick matches 23 if entity @s[tag=RW.HPless75per] run function asset:mob/1004.tultaria/tick/1.melee_attack/4.slash2
