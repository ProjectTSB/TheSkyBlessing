...
# 自分を透明化する
    execute if score @s 1N.Tick matches 0 run item replace entity @s armor.head with air

# 自分を何処かにテレポートさせる
    execute if score @s 1N.Tick matches 5 run function asset:mob/0059.jack_o_lantern/tick/spread_tp
...
# hurt時につく被ダメタグがあった場合、クイズに正解して、このスキルを終える
    execute if score @s 1N.Tick matches 20..159 if entity @s[tag=1N.Hurt] run function asset:mob/0059.jack_o_lantern/tick/1.skill_quiz/quiz_end
