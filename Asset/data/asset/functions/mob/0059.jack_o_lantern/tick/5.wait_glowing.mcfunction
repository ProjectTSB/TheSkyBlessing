...
# ワープ
    ...
    execute at @p[gamemode=!creative,gamemode=!spectator] positioned ~ ~20 ~ rotated ~ 90 run function lib:forward_spreader/circle
    execute at @p[gamemode=!creative,gamemode=!spectator] positioned ~ ~20 ~ facing entity 0-0-0-0-0 feet positioned ^ ^ ^20 if block ~ ~ ~ #asset:mob/0059.jack_o_lantern/no_collision unless block ~ ~-1 ~ #asset:mob/0059.jack_o_lantern/no_collision run tp @s ~ ~ ~
# ワープミスによる特殊攻撃割愛
