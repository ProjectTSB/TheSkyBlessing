#> asset:mob/0013.cluster_creeper/summon/2.summon
#
# Mobの召喚時の処理
#
# @within function asset:mob/0013.cluster_creeper/summon/1.trigger

# 元となるMobを召喚する
    summon creeper ~ ~ ~ {Tags:["MobInit"],DeathLootTable:"asset:mob/death/0013.cluster_creeper"}
# ID (int)
    data modify storage asset:mob ID set value 13
# Type (string) Wikiを参照
    data modify storage asset:mob Type set value "Enemy"
# 干渉可能か否か (boolean)
    data modify storage asset:mob Interferable set value true
# 名前 (TextComponentString) (オプション)
    data modify storage asset:mob Name set value '{"text":"クラスタークリーパー","color":"green"}'
# 武器
...
# 体力 (double) (オプション)
    data modify storage asset:mob Health set value 1.0
# 攻撃力 (double) (オプション)
...
# 移動速度 (double) (オプション)
    data modify storage asset:mob Speed set value 0.50
# 索敵範囲 (double) (オプション)
...
# MobInitタグ持ちを対象にして召喚関数呼び出し
    execute as @e[type=creeper,tag=MobInit,distance=..0.01] run function asset:mob/common/summon