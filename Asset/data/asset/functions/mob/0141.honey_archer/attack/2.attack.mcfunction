...
# デスログ
    data modify storage lib: Argument.DeathMessage set value ['{"translate": "%1$sは%2$sによってべとべとに射貫かれた","with":[{"selector":"@s"},{"nbt":"Return.AttackerName","storage":"lib:","interpret":true}]}','{"translate": "%2$sによって%1$sはハチの巣の材料にされた","with":[{"selector":"@s"},{"nbt":"Return.AttackerName","storage":"lib:","interpret":true}]}']
# 補正functionを実行
