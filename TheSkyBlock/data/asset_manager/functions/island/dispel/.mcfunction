...
# プレイヤー座標のチェック
    ...
    execute unless score $nonPosEqual Temporary matches 0 store success score $nonPosEqual Temporary run data modify storage oh_my_dat: _[-4][-4][-4][-4][-4][-4][-4][-4].PlayerPos set from storage api: Pos
# 座標が変わってる場合はリセット
...
# 解呪時間の3/4のタイミングでボス召喚フラグが立っていない場合召喚する
    execute if score @s DispelTime matches 120 if data storage oh_my_dat: _[-4][-4][-4][-4][-4][-4][-4][-4].IslandData{BossSummoned:false} run function asset_manager:island/dispel/summon_boss/
# 解呪時間を満たした場合解呪する
