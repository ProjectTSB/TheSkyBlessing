...
# 条件を満たしてたらtagを付与
    # MP必要量による制限
        execute store result score $CheckMP Argument run data get storage asset:sacred_treasure Item.tag.TSB.MPRequire
        function player_manager:mp/check
        execute if score $HaveMP Argument matches 1 run tag @s add PassConditionA
        scoreboard players reset $haveMP Argument
    # 信仰による制限
