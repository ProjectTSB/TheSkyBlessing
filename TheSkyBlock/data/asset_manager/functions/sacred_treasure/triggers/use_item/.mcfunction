...
#> consumable判定用アマスタ
# @private
#declare tag ForPredicateArmorStand
...
# 食べたスロットを特定する
    ...
    # アイテムが食べれるかで特定を試みる
        execute unless data storage asset:sacred_treasure Argument.AutoSlot run summon armor_stand 0.0 0.0 0.0 {Marker:1b,Invisible:1b,Tags:["ForPredicateArmorStand"]}
        execute unless data storage asset:sacred_treasure Argument.AutoSlot as @e[type=armor_stand,tag=ForPredicateArmorStand,distance=..0.001,x=0.0,y=0.0,z=0.0,limit=1] run function asset_manager:sacred_treasure/triggers/use_item/check_has_consumable
# イベント発火前に実行するやつ
