#> entity_manager:lib/score_damage/core/health_subtract
#
# MobのHealthを減算する
#
...
# 演出
    execute if score $Health Temporary matches 1.. if entity @s[type=#entity_manager:lib/score_damage/undead] run effect give @s instant_health 1 31 true
    execute if score $Health Temporary matches 1.. if entity @s[type=!#entity_manager:lib/score_damage/undead] run effect give @s instant_damage 1 31 true
    execute if score $DisableParticle Temporary matches 0 at @s run function entity_manager:lib/score_damage/core/damage_indicator
# リセット
