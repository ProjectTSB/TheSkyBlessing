...
# 演出
    execute if score $Health Temporary matches 1.. if entity @s[type=#lib:damage/undead] run effect give @s instant_health 1 31 true
    execute if score $Health Temporary matches 1.. if entity @s[type=!#lib:damage/undead] run effect give @s instant_damage 1 31 true
