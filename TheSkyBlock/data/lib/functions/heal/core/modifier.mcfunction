#> lib:heal/core/modifier
#
#
#
# @within function lib:heal/modifier

#> Temp
# @private
    #declare score_holder $Heal
    #declare score_holder $Modifier

# ユーザーストレージ呼び出し
    function oh_my_dat:please
# 必要なデータ取得
    execute store result score $Heal Temporary run data get storage lib: Argument.Heal 10000
    execute store result score $Modifier Temporary run data get storage oh_my_dat: _[-4][-4][-4][-4][-4][-4][-4][-4].Modifier.Heal 10000
# 補正
    scoreboard players operation $Heal Temporary *= $Modifier Temporary
# 代入
    execute store result storage lib: Argument.Heal float 0.00000001 run scoreboard players get $Heal Temporary
# リセット
    scoreboard players reset $Heal Temporary
    scoreboard players reset $Modifier Temporary