...
# Operationを見て別に配列に移す
    execute if data storage api: {Op:"add"} store result score $Temp Temporary run data get storage api: Modifiers[-1].Amount 10
    execute if data storage api: {Op:"add"} run scoreboard players operation $Add Temporary += $Temp Temporary
    execute if data storage api: {Op:"multiply_base"} store result score $Temp Temporary run data get storage api: Modifiers[-1].Amount 10
