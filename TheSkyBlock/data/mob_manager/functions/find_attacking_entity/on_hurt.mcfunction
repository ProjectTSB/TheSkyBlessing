#> mob_manager:find_attacking_entity/on_hurt
#
# 攻撃対象を特定します
#
...
# タグ付け
    tag @s add this
# 対象を設定してフィルターにかける
    execute as @e[type=#lib:living,type=!player,distance=..150] run function mob_manager:find_attacking_entity/filters/15
# リセット
    tag @s remove this
    ...
    advancement revoke @s only mob_manager:find_attacking_entity/check_hurt_entity