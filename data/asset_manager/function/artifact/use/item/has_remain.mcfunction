...
# 対象スロットの残り使用回数取得
    execute store result score $Remain Temporary run data get storage asset:artifact TargetItems[-1].components."minecraft:custom_data".TSB.RemainingCount
# 減算
    scoreboard players remove $Remain Temporary 1
# アイテム更新処理
    execute store result storage asset:artifact TargetItems[-1].components."minecraft:custom_data".TSB.RemainingCount int 1 run scoreboard players get $Remain Temporary
