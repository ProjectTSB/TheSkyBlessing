...
# エラー値
    ...
    execute if score @s Believe4 = $Difficulty Global if score @s Believe4 matches 1..3 run function lib:message/invalid_operation
# ロック
...
# 正常値
    ...
    execute if entity @s[tag=Difficulty.Request] if entity @a[tag=!Difficulty.Request] run function player_manager:god/change_difficulty/confirm/
    execute if entity @s[tag=Difficulty.Request] unless entity @a[tag=!Difficulty.Request] run function player_manager:god/change_difficulty/confirm/approve/change
# リセット
