...
# 防御力
    execute if data storage lib: Argument{BypassDefense:false} store result score $defensePoints Temporary run attribute @s generic.armor get 100
    execute if data storage lib: Argument{BypassDefense: true} run scoreboard players set $defensePoints Temporary 0
# 防具強度
...
# エンチャント値の取得
    execute if data storage lib: Argument{BypassEnchantment:false} run function lib:damage/core/get_epf/
    execute if data storage lib: Argument{BypassEnchantment: true} run scoreboard players set $EPF Temporary 0
# 耐性エフェクト
