...
# 引数チェック
    ...
    execute unless data storage lib: Argument.BoundingFan.Height run return run tellraw @a [{"storage":"global","nbt":"Prefix.ERROR"},{"text":"lib:bounding_fan/","color":"gold"},{"text":"の引数が不足しています。","color":"white"}]
    execute unless data storage lib: Argument.BoundingFan.Selector run return run tellraw @a [{"storage":"global","nbt":"Prefix.ERROR"},{"text":"lib:bounding_fan/","color":"gold"},{"text":"のHeight引数が不足しています。","color":"white"}]
