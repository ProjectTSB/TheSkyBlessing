...
#   storage lib: Argument.SlideMove.ThroughBlock: String 通れるブロックorブロックタグ（省略した場合は"#lib:no_collision/"）
# @api
#
#> temp
...
# 引数チェック
    ...
    execute unless data storage lib: Argument.SlideMove.ThroughBlock run data modify storage lib: Argument.SlideMove.ThroughBlock set value "#lib:no_collision/"
...
# 引数コピー
    ...
    data modify storage lib: SlideMove.Macro.ThroughBlock set from storage lib: Argument.SlideMove.ThroughBlock
...
# 後片付け
    ...
    data remove storage lib: Argument.SlideMove.ThroughBlock
