...
# validate
    ...
    execute if data storage global {IsProduction:false} unless loaded ~ ~ ~ run tellraw @a [{"storage":"global","nbt":"Prefix.ERROR"},{"text":"ロードされていない座標に対して Mob の召喚が行われました","color":"white"}]
# 呼び出し
