...
# イベントの追加
    execute if data storage api: Argument{PushEvent:true} run function api:damage/core/trigger_events/non-player/attack_and_hurt/
    execute if data storage api: Argument{PushEvent:true} if score $Health Temporary matches ..0 run function api:damage/core/trigger_events/non-player/kill_and_death/
