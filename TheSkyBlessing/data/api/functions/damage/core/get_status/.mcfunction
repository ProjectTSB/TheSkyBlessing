...
# ダメージ
    execute if data storage lib: Argument{BypassModifier:false} run function api:damage/core/get_status/modify_and_get_damage
...
# エンチャント値の取得
    execute if data storage lib: Argument{BypassEnchantments:false} run function api:damage/core/get_epf/
    execute if data storage lib: Argument{BypassEnchantments: true} run scoreboard players set $EPF Temporary 0
# 耐性エフェクト
    execute if data storage lib: Argument{BypassResistance:false} run function api:damage/core/get_status/get_resistance_lv
