...
# 初回の攻撃か判別する
    execute store result storage api: Args.ID int 1 run scoreboard players get @s DamageForwardTargetMobUUID
...
# ダメージを与える
    execute if data storage api: {IsInitialAttack:true} run scoreboard players operation $DamageForwardTarget Temporary = @s DamageForwardTargetMobUUID
