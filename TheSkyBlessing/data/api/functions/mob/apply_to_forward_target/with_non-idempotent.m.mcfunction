...
# 対象が拡張された当たり判定でなければそのまま実行する
    $execute if entity @s[tag=!ExtendedCollision] run return run function api:mob/apply_to_forward_target/with_idempotent.m {CB:"$(CB)"}
...
# 呼び出し
    $execute if data storage api: {IsInitialApply:true} run function api:mob/apply_to_forward_target/with_idempotent.m {CB:"$(CB)"}
# リセット
