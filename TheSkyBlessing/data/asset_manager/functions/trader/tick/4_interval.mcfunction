...
# 奈落回避
    execute if data storage global {IsProduction:1b} as @e[type=villager,tag=Npc,tag=Trader] at @s if predicate lib:is_void run function asset_manager:trader/to_spawn