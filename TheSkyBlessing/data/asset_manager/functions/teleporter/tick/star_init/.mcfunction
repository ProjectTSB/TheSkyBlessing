...
# グループIDから対象TP先を取得する
    ...
    execute if data storage asset:teleporter TargetGroupID run function asset_manager:teleporter/tick/data/pick
# 破壊的変更を行うとまずいのでコピーする
    data modify storage asset:teleporter Teleporters set from storage asset:teleporter TeleporterGroup.Teleporters
# 星を召喚する
    execute if data storage asset:teleporter Teleporters[0] anchored eyes positioned ^ ^ ^ run function asset_manager:teleporter/tick/star_init/foreach
# テレポート先が無いならメッセージ
