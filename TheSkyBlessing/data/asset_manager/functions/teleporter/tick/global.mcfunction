...
# 天使確認
    execute if data storage asset:teleporter Teleporters[-1].Data{AngelProtection:     true} unless entity @e[type=#lib:living,tag=Enemy.Boss,distance=..120] run function asset_manager:teleporter/tick/angel_protection/disable
    execute unless data storage asset:teleporter Teleporters[-1].Data{AngelProtection: true} if entity @e[type=#lib:living,tag=Enemy.Boss,distance=..120] run function asset_manager:teleporter/tick/angel_protection/enable
