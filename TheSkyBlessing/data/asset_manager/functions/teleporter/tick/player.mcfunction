...
# テレポート前の処理
    execute if entity @s[tag=!Teleported,predicate=lib:is_sneaking] if entity @e[type=marker,tag=FromTeleporter,distance=..1,limit=1] if data storage oh_my_dat: _[-4][-4][-4][-4][-4][-4][-4][-4].TeleporterData{ActivationKind:"Activate"} run function asset_manager:teleporter/tick/active
# テレポート後のタグ解除処理
