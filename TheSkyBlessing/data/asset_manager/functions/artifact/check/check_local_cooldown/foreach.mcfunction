...
# 最後に使用したtickとLocalCooldownを取得
    execute store result score $LatestUsedTick Temporary run data get storage asset:artifact CopiedItemList[-1].LatestUseTick
    execute store result score $Cooldown Temporary run data get storage asset:artifact CopiedItemList[-1].LocalCooldown
# 減算
    scoreboard players operation $Tick Temporary -= $LatestUsedTick Temporary
# 比較
    execute if score $Tick Temporary >= $Cooldown Temporary run data modify storage asset:artifact TargetItemList append from storage asset:artifact CopiedItemList[-1]
...
# 要素がまだあるなら再帰
    execute if data storage asset:artifact CopiedItemList[0] run function asset_manager:artifact/check/check_local_cooldown/foreach
