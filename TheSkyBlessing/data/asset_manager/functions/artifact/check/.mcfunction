...
# 条件を満たしてなかったらtag削除
    # 信仰による制限
        execute unless data storage asset:artifact TargetItems[0].tag.TSB{DisabledCheckFlags:["Believe"]} run function asset_manager:artifact/check/check_believe
    ...
    # SpecialCooldownによる制限
        execute unless data storage asset:artifact TargetItems[0].tag.TSB{DisabledCheckFlags:["SpecialCooldown"]} run function asset_manager:artifact/check/check_special_cooldown
        ...
        tag @s[tag=CheckFailed] remove CheckFailed
    # MP必要量による制限
        execute unless data storage asset:artifact TargetItems[0].tag.TSB{DisabledCheckFlags:["MPRequire"]} run function asset_manager:artifact/check/check_mp
    ...
    # SecondaryTypeCooldownによる制限
        ...
        tag @s[tag=CheckFailed] remove CheckFailed
    # LocalCooldownによる制限
        execute unless data storage asset:artifact TargetItems[0].tag.TSB{DisabledCheckFlags:["LocalCooldown"]} run function asset_manager:artifact/check/check_local_cooldown/
