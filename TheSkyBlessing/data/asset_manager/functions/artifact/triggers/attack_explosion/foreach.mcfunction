#> asset_manager:artifact/triggers/attack_explosion/foreach
#
#
#
# @within function asset_manager:artifact/triggers/attack_explosion/*
...
# 使用条件を満たしているか確認する
    ...
    function asset_manager:artifact/triggers/attack_explosion/check.m with storage asset:context
# 条件を満たしていれば使用する
    execute if entity @s[tag=CanUsed] run function asset_manager:artifact/triggers/attack_explosion/use
...
# 要素が残っているなら再帰
    execute if data storage asset:artifact Events[0] run function asset_manager:artifact/triggers/attack_explosion/foreach
