...
# フラグが存在しているか確認する
    execute unless data storage asset:artifact ArtifactEvents.Attack[{Type:"vanilla_projectile"}] run return fail
# イベントを取得する
    data modify storage asset:artifact Events append from storage asset:artifact ArtifactEvents.Attack[{Type:"vanilla_projectile"}]
# それぞれについて処理する
