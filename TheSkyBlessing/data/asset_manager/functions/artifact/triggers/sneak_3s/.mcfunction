#> asset_manager:artifact/triggers/sneak_3s/
#
# 各神器処理へデータ受け渡し
#
...
# 使用条件を満たしているか確認する
    ...
    execute if score $SneakTime Temporary matches 60 run function asset_manager:artifact/triggers/sneak_3s/check.m with storage asset:context
# 条件を満たしていれば使用する
    execute if entity @s[tag=CanUsed] run function asset_manager:artifact/triggers/sneak_3s/use
