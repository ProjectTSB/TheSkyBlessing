...
# データ取得
    ...
    execute store result score $Max Temporary run data get storage asset:artifact CopiedItemData[-1].LocalCooldown
    execute store result score $LatestUsedTick Temporary run data get storage asset:artifact CopiedItemData[-1].LatestUseTick
# 使用してからの時間を取得
...
# 設定
    ...
    execute store result storage lib: Array[-1].Max int 1 run data get storage asset:artifact CopiedItemData[-1].LocalCooldown
# 末尾削除
