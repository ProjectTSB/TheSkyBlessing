#> asset_manager:artifact/triggers/equipments/set_and_modifier/add/stack/
# @within function
...
# Modifierに値を追加
    ...
    execute if data storage asset:artifact Temp.CurrentDifferentEquipments[-1].Equipment.Modifiers[0] run data modify storage asset:artifact Temp.CurrentDifferentEquipments[-1].Equipment.Modifiers[].MaxStack set from storage asset:artifact Temp.CurrentDifferentEquipments[-1].MaxStack
...
# Modifierのスタック処理
    execute if data storage asset:artifact Temp.ModifierStack[0] run function asset_manager:artifact/triggers/equipments/set_and_modifier/add/stack/foreach
# 要素を削除して、残っているならば再帰
