...
# Modifierに値を追加
    ...
    execute if data storage asset:artifact Temp.CurrentDifferentEquipments[-1].Equipment.Modifiers[0] run data modify storage asset:artifact Temp.CurrentDifferentEquipments[-1].Equipment.Modifiers[].Slot set from storage asset:artifact Temp.CurrentDifferentEquipments[-1].Equipment.Slot
    execute if data storage asset:artifact Temp.CurrentDifferentEquipments[-1].Equipment.Modifiers[0] run data modify storage asset:artifact Temp.CurrentDifferentEquipments[-1].Equipment.Modifiers[].MaxStack set from storage asset:artifact Temp.CurrentDifferentEquipments[-1].Equipment.MaxStack
