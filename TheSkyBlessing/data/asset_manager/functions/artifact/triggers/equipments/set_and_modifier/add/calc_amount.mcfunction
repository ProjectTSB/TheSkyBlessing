#> asset_manager:artifact/triggers/equipments/set_and_modifier/add/calc_amount
...
# 変数を取得
    execute store result score $Stack Temporary run data get storage asset:artifact Temp.Modifier.Stack
    execute store result score $Amount Temporary run data get storage asset:artifact Temp.Modifier.Amount 1000
    execute store result score $MaxStack Temporary run data get storage asset:artifact Temp.Modifier.MaxStack
    execute store result score $StackReduction Temporary run data get storage asset:artifact Temp.Modifier.StackReduction 1000
...
# 計算
    ...
    execute store result storage asset:artifact Temp.Modifier.Amount double 0.001 run scoreboard players get $Amount Temporary
# リセット
