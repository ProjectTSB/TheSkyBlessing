#> asset_manager:artifact/triggers/sneak_2s/
#
# 各神器処理へデータ受け渡し
#
...
# 使用条件を満たしているか確認する
    ...
    execute if score $SneakTime Temporary matches 40 run function asset_manager:artifact/triggers/sneak_2s/check.m with storage asset:context
# 条件を満たしていれば使用する
    execute if entity @s[tag=CanUsed] run function asset_manager:artifact/triggers/sneak_2s/use
