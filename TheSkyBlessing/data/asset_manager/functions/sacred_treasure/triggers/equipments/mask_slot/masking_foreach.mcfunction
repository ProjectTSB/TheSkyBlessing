...
# 末尾要素に応じて追加するものを変える
    execute if data storage asset:sacred_treasure CopiedEquipChanges[-1]._{_: true} run data modify storage asset:sacred_treasure MaskedEquips append value -1
    execute if data storage asset:sacred_treasure CopiedEquipChanges[-1]._{_:false} run data modify storage asset:sacred_treasure MaskedEquips append from storage asset:sacred_treasure Equipments[-1]
# 末尾削除
