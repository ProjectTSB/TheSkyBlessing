...
# 条件を満たしてなかったらtag削除
    ...
    # LocalCooldownによる制限
        ...
        execute if entity @s[tag=CheckFailed] unless score @s LocalCDLogCD matches 0.. unless data storage asset:sacred_treasure Item.tag.TSB{DisableCooldownMessage:true} run tellraw @s {"text":"クールダウンが終わっていません。","color":"red"}
        execute if entity @s[tag=CheckFailed] unless score @s LocalCDLogCD matches 0.. unless data storage asset:sacred_treasure Item.tag.TSB{DisableCooldownMessage:true} run scoreboard players set @s LocalCDLogCD 20
        execute if entity @s[tag=CheckFailed] run tag @s remove CanUsed
        tag @s[tag=CheckFailed] remove CheckFailed
