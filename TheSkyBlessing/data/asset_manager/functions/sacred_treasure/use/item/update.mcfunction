...
# データ更新処理
    data modify storage asset:sacred_treasure Name set from storage asset:sacred_treasure Item[-1].tag.TSB.rawName
    # 残り回数が存在する場合
        execute if data storage asset:sacred_treasure Item[-1].tag.TSB.RemainingCount run loot replace block 10000 0 10000 container.0 loot asset_manager:sacred_treasure/get_name/has_remain
    # 残り回数が存在しない場合
        execute unless data storage asset:sacred_treasure Item[-1].tag.TSB.RemainingCount run loot replace block 10000 0 10000 container.0 loot asset_manager:sacred_treasure/get_name/
    # 不要な破壊的変更を阻止するためにコピーして生成する
        data modify storage asset:sacred_treasure CopyItem set from storage asset:sacred_treasure Item[-1]
...
# 新しい神器で上書きする
    ...
    execute if data storage asset:sacred_treasure {TargetDefaultSlot:"hotbar"} store result storage api: SelectedItemSlot int 1 run data get storage asset:sacred_treasure Item[-1].Slot
...
# current更新
    data modify storage asset:sacred_treasure ProcessedItem set from storage asset:sacred_treasure Item[-1]
