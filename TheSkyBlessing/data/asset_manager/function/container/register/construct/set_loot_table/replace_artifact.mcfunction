...
# それぞれについて修正
    execute if data storage asset:container ArtifactSeeds[-1].components."minecraft:custom_data".TSB.ArtifactSeed.ID run function asset_manager:container/register/construct/common/get_artifact.m {IDPath:"ArtifactSeeds[-1].components."minecraft:custom_data".TSB.ArtifactSeed.ID"}
    execute if data storage asset:container ArtifactSeeds[-1].components."minecraft:custom_data".TSB.ArtifactSeed.Rarity run function asset_manager:container/register/construct/set_loot_table/roll_artifact
