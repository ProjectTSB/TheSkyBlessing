...
# 溶岩ダメージのクールダウン
    execute if predicate lib:in_lava/include_flowing run scoreboard players add @s LavaDamageCooldown 1
    execute if predicate lib:in_lava/include_flowing if score @s LavaDamageCooldown matches 10.. run scoreboard players reset @s LavaDamageCooldown

# 溶岩の外に出たならクールダウンをリセット
    execute unless predicate lib:in_lava/include_flowing run scoreboard players reset @s LavaDamageCooldown
